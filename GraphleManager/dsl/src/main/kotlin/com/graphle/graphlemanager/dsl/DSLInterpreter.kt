...
import com.graphle.graphlemanager.application.FileDetailsService
 ...
 * @param fileDetailsService Application service for cross-domain file details
 */
@Service
            ...
            when (tokens.first()) {
                ...
                Command.DETAIL.command -> {
                    val filename = tokenInterpreter.parseDetailTokens(tokens.drop(1))
                        ?: return parseError(command)
                    val detail: File = fileDetailsService.fileByLocation(filename) ?: return DSLResponse(
                        ResponseType.ERROR,
