...
The system is composed of five components: GraphleManager, GraphleUI, the DSL client, Neo4j, and Valkey.
The application is not purely RESTful.
GraphleManager exposes three distinct communication interfaces — GraphQL, REST, and WebSocket — each chosen for the specific needs of the client that uses it.
...
It hosts the DSL interpreter, manages both external datastores, and exposes the three communication interfaces described below.
Because GraphleManager is a network service, clients can connect to an instance running on a remote machine, giving users access to a remote filesystem (Q1.3).
...
It communicates with GraphleManager exclusively through the public API, which means it can be replaced by any third-party client that speaks the same interfaces (Q1.2).
The default client ships with both a light and a dark theme (Q1.2).
File and folder manipulation is exposed through GraphQL mutations for the GUI and through the DSL for the command line client (F4, F5).

*DSL client* is a command line tool that sends commands to GraphleManager over REST (F7).
It is the second user interface available to users alongside the GUI (Q1.1).

*Neo4j* stores the graph of files, relationships, and tags as a Labeled Property Graph (F2, F3).
...
*Valkey* holds the filename autocomplete index in memory as a trie structure (F8).
GraphleManager populates and queries this index on behalf of clients.
Frequently accessed trie nodes are additionally cached inside GraphleManager using a set of concurrent caches, so hot nodes are served without a network round trip to Valkey.
If Valkey is unavailable, autocomplete is silently disabled while all other operations continue (Q3.2).

=== Communication Interfaces

*GraphQL* (`/graphql`) is used by GraphleUI for all file, tag, and connection operations.
...
*REST* (`/dsl`, `/download`) is used by the DSL client and by GraphleUI for DSL command execution and file downloads.
REST was chosen for these endpoints because issuing an HTTP request from the command line is straightforward, whereas constructing a GraphQL query would add unnecessary overhead for CLI use.

*WebSocket* (`/ws`) provides a persistent connection for DSL autocomplete.
...
They are loaded from the filesystem on demand the first time a query requests them (F9).
...
Parent and descendant relationships are resolved live by reading the OS filesystem hierarchy rather than being stored in Neo4j, 
...
It fetches all file locations recorded in Neo4j, checks whether each path still exists on the filesystem,
and removes any that have been deleted outside the application along with their orphaned tags and relationships.
This keeps the database consistent with the underlying filesystem without requiring the user to manually clean up deleted files.
