...
=== Autocomplete latency measurement <autocomplete-latency>
...
The client timed each autocomplete request from sending the prefix frame to receiving the matching response frame on an already established local `/ws` connection.
The samples therefore exclude connection setup and internet latency. Instead, they focus on request handling. This includes Valkey lookup, path reconstruction, filesystem checks, response transfer, and client-side parsing.
