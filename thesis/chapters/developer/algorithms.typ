...
Autocomplete is implemented as a #link(label("voc_trie"))[trie] storing paths. The trie is stored in #link(label("voc_valkey"))[Valkey], with an in-process cache in front of it to save on round trips to the database.
The implementation lives in `dsl/FilenameCompleter.kt` and utilizes a `Storage` abstraction so the trie logic can be tested against a fake or swapped for a different storage backend.

Each trie node is identified by a numeric index, where the root is assigned index 0.
Each node stores its parent link back up the trie, the character it represents, a set of back-references to full-path nodes (used for the reverse-lookup described below), and a flag indicating whether it terminates a complete path.
...
The insertion path implements the "modified trie" described in the architecture chapter.
...
Memory therefore grows with the number of unique directory and filename components in the dataset, rather than with the number of files multiplied by their average depth. This matters because the in-process cache stores trie nodes directly. Duplicated prefixes would fill the cache with redundant entries and reduce its effectiveness.
...
Five dedicated caches, one per access pattern, sit in front of the storage layer.
Each cache entry is refreshed on every access (whether a hit or a miss), so hot nodes stay warm without a separate TTL refresh step. The result is that the first read after startup and all subsequent reads produce the same in-memory view of the data.
...
This allows a query to express graph traversals incrementally, alternating between conditions on files and conditions on the relationships that connect them.
