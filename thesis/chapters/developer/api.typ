...
#table(
  ...
  [`addTagToFile(location, tag)`], [Creates or updates the tag node and its link to the file.],
  [`removeTag(location, tag)`], [Removes the tag link and garbage-collects the tag node if it becomes orphaned.],
...
GraphleManager is intended to run on the operator's own machine or on a trusted LAN host under their control, which matches the remote-access requirement (Q1.3) without exposing the filesystem to untrusted callers.
