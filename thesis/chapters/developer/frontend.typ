...
Backend access is split between two transports.
#voc("apollo") handles all file, #voc("tag"), and #voc("connection") operations.
Ktor handles the #voc("dsl") #voc("rest") call and the autocomplete #voc("websocket").
...
- `common/`: configuration, singletons, the #voc("graphql") wrapper, a trash helper, the UI state models, and the shared theme and reusable visual primitives.
...
- `dialogs/`: the modal flows for create/delete/move/#voc("tag")/#voc("relationship") actions and for surfacing error and invalid-file messages, plus a composite that mounts them inside `App`.
- `header/`: the search header (command line, theme toggle, menu) and the #voc("rest") and #voc("websocket") transports it drives.
- `dsl/`: command history and the response dispatcher that turns a #voc("dsl") reply into a display mode change.

=== Configuration

The GUI reads a YAML configuration file with a single `server` section that exposes two options:

- `port`: the TCP port on which the backend is listening. Used to build the #voc("graphql"), #voc("rest"), download, and #voc("websocket") URLs that the transports target.
- `localhost`: a boolean indicating whether the backend runs on the same host as the GUI. When true, the file opening flow skips the #voc("http") download step and opens the file in place. When false, the GUI always pulls a local copy through `/download` first.
...
`DisplayedSettings` wraps a nullable `DisplayedData` (the currently visible entities: filenames, #voc("tag", text: "tags"), #voc("connection", text: "connections"), or a file detail) and a `DisplayMode` enum that selects which body to render.
...
`DSLRestManager` posts #voc("dsl") commands to `POST /dsl` on the configured backend and deserialises the `DSLResponse`.
