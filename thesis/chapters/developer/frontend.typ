...
Backend access is split between two transports: #link(label("voc_apollo"))[Apollo] #link(label("voc_graphql"))[GraphQL] for all file, #link(label("voc_tag"))[tag], and #link(label("voc_connection"))[connection] operations, and Ktor for the #link(label("voc_dsl"))[DSL] #link(label("voc_rest"))[REST] call and for the autocomplete #link(label("voc_websocket"))[WebSocket].
...
- `common/`: configuration, singletons, the #link(label("voc_graphql"))[GraphQL] wrapper, a trash helper, the UI state models, and the shared theme and reusable visual primitives.
...
- `dialogs/`: the modal flows for create/delete/move/#link(label("voc_tag"))[tag]/#link(label("voc_relationship"))[relationship] actions and for surfacing error and invalid-file messages, plus a composite that mounts them inside `App`.
- `header/`: the search header (command line, theme toggle, menu) and the #link(label("voc_rest"))[REST] and #link(label("voc_websocket"))[WebSocket] transports it drives.
- `dsl/`: command history and the response dispatcher that turns a #link(label("voc_dsl"))[DSL] reply into a display mode change.

=== Configuration

The GUI reads a YAML configuration file with a single `server` section that exposes two options:

- `port`: the TCP port on which the backend is listening. Used to build the #link(label("voc_graphql"))[GraphQL], #link(label("voc_rest"))[REST], download, and #link(label("voc_websocket"))[WebSocket] URLs that the transports target.
- `localhost`: a boolean indicating whether the backend runs on the same host as the GUI. When true, the file opening flow skips the #link(label("voc_http"))[HTTP] download step and opens the file in place. When false, the GUI always pulls a local copy through `/download` first.
...
`DSLRestManager` posts #link(label("voc_dsl"))[DSL] commands to `POST /dsl` on the configured backend and deserialises the `DSLResponse`.
